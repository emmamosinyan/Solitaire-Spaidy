...
import { EVT_HITTABLE_REGISTER, EVT_HITTABLE_UNREGISTER, EVT_PLAY_SOUND, SOUND_CARD_MOVE, EVT_FOUNDATION_COMPLETED } from 'db://assets/scripts/common/events';
...
export class CardStack extends Component {
    ...
    private onCardAdded(card: Card): void {
        ...
        // Первая карта (туз) попала в foundation — открываем замок
        if (this.stackType === StackType.FOUNDATION && this.cards.length === 1) {
            GlobalEventBus.publish({ type: EVT_FOUNDATION_COMPLETED, suit: this.targetSuit });
        }
    }
